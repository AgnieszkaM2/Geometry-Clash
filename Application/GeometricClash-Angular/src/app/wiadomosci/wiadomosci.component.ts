...
export class WiadomosciComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.loggedIn) {this.loadMessages();}
  }
  ...
  loadMessages() {
    this.memberService.getMember(this.user.username).subscribe(member => {
      this.member = member;
      this.getMessagesPath = this.auth.BaseURL + "/Messages/Received?id=" + member.id.toString();
      this.getMessages();
      this.ChangeReadMessagesStatus();
    })
  }
  ChangeReadMessagesStatus() {
    this.http.put(this.auth.BaseURL + "/Messages?id=" + this.member.id, null).subscribe(() => {}, error => {
      console.log(error);
    })
  }
  logout() {
